...
def set_ratios(g,marked):
    ...
    for i in range(len(non_leaves)):
        ...
        this_pos_leaves = [l for l in this_leaves if(g.node[l]['marked'] == True)]
        g.node[non_leaves[i]]['ratio'] = len(this_pos_leaves)/len(this_leaves)
...
def thr_all_graphs(g,marked,thr):
    ...
    with progressbar.ProgressBar(maxval=len(g)) as bar:
        for f in range(len(g)):
            ...
            new_m = np.concatenate((np.tile(f,new_m.shape[0]).reshape(-1,1),
                                    new_m.reshape(-1,1)),axis=1).astype(int)
...
def get_merge_candidates(g,candidates,thr):
    ...
    #for i in np.arange(2, len(children) + 1):
        ...
        redraw = False
        print('n_candidates, K, len(combs): ' + str(len(candidates)) + ',' + str(K) + ',' + str(len(this_combs)))
        for j in range(len(this_combs)):
            ...
            sum_elems_candidates = np.sum([n_elems[this_combs[j][c]] for c in range(len(this_combs[j]))])
            #Compute ratio of combination with all common parents
            ratios = np.asarray([sum_elems_candidates/(n_elems[common_parents[l]]) for l in range(len(common_parents))])
            ...
            if(idx_can_merge.size > 0):
                ...
                candidates = [c for c in candidates if(c not in this_combs[j])]
                redraw = True
                break
        if(redraw == False):
            K -= 1
...
def hierarchical_segmentation(I, feature_mask = features.SimilarityMask(1, 1, 1, 1,1),F0=None, desc_arr = None,k=100, return_stacks=False):
    """
    I: Image
...
def selective_search(I, color_spaces = ['rgb'], ks = [100], feature_masks = [features.SimilarityMask(1, 1, 1, 1, 1)], n_jobs = -1):
